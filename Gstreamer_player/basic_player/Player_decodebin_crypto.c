...
typedef struct play{
	...
	GstElement *audioqueue,*videoqueue,*crypto_queue,*cryptoo;
	GstState state;
	GMainLoop *loop;
	GtkWidget *slider;            
...
int main(int argc, char *argv[])
{
	...
	data.crypto_queue= gst_element_factory_make ("queue", "cryqueue");
 	...
 	gst_bin_add_many (GST_BIN (data.pipeline),data.source,data.cryptoo,data.crypto_queue,data.decodebinn,data.videoqueue, data.Vconv,data.Vsink,data.audioqueue, data.Aconv , data.Asink, data.Are_sa, NULL);//add ele in bin

	 gst_element_link_many(data.source,data.cryptoo,data.crypto_queue, data.decodebinn,NULL);
	 ...
	 if(argc>=2)
		...
		g_object_set (G_OBJECT (data.source), "location","/home/kakanhar/Videos/encrypted/sample.png.enc", NULL);
