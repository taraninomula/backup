...
static void create_gui (Playdata *data) 
{
	...
	*zoom_out_button,*previous_button,*audio_mixer_button;
	...
	g_signal_connect (G_OBJECT (folder_button), "clicked", G_CALLBACK (folder_cb), data);//audio_mixer_button
	
	audio_mixer_button = gtk_button_new_from_icon_name ("audio-x-generic", GTK_ICON_SIZE_SMALL_TOOLBAR);
	//g_signal_connect (G_OBJECT (folder_button), "clicked", G_CALLBACK (audio_mixer_cb), data);//audio_mixer_button
	...
	gtk_widget_set_name(audio_mixer_button, "myButton_cyan");
	...
	gtk_box_pack_start (GTK_BOX (controls_UP), audio_mixer_button, FALSE, FALSE,2);
...
static void folder_cb(GtkButton *button, Playdata *data) 
{
	GtkWidget *dialog; static char *filename=NULL,*uri=NULL;
	gst_element_set_state (data->pipeline, GST_STATE_PAUSED);
